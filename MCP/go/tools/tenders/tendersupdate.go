...
func TendersupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/pos/tenders/%s%s", cfg.BaseURL, id, queryString)
		...
		var result models.UpdateTenderResponse
...
func CreateTendersupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_pos_tenders_id",
		mcp.WithDescription("Update Tender"),
		...
		mcp.WithBoolean("opens_cash_drawer", mcp.Description("Input parameter: If this tender opens the cash drawer")),
		mcp.WithString("updated_at", mcp.Description("Input parameter: The date and time when the object was last updated.")),
		mcp.WithBoolean("allows_tipping", mcp.Description("Input parameter: Allow tipping on payment from tender")),
		...
		mcp.WithBoolean("editable", mcp.Description("")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    TendersupdateHandler(cfg),
	}
}
